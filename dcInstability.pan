...
parameters TCROSS=10n TSTOP=40
options rawkeep=yes topcheck=2 compile=1 outintnodes=yes
...
Save control begin 
    ...
    Time = get("Tr.time");
    L    = get("Tr.Ldc:L");
    VdcP = get("Tr.BmC1_pos");

    save("mat5", "dcInstability.mat", "time", Time, "L", L, "VdcP", VdcP );
...
Ldc    xBmC1_pos   BmC1_pos  IND
;Ldc    BmC1_pos   xBmC1_pos  ccvs sensedev=CfC1 func= \
;      (50m + 30m * min(1, max(0, time-5)/(30-5)))*2*ddt(i(CfC1))

CfC1   xBmC1_pos   BmC1_neg  vsource dc=VDC_NOM 

model IND nport veriloga="ind.va" verilogatrace=[L]
